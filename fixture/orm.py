...
class ORMFixture:
    ...
    def convert_grps_to_model(self, groups):
        def convert(group):
            return Group(id=str(group.id), name=group.name, header=group.header, footer=group.footer)
        return list(map(convert, groups))

    @db_session  # необходимо указать что этот блок кода должен выполняться в рамках сессии
    def get_grp_list(self):
        return self.convert_grps_to_model(select(g for g in ORMFixture.ORMGroup))
